...
Three numbers were corrected at ratification — they override older figures elsewhere in the vault:

1. **VST asymmetry is ~1.1x, not 13x.** The old 13x used the too-tight $145 bear (only ~4% under the prior entry). Using the real bear in the [[VST]] note (**$105**: mild weather + Cogentrix delayed + $6.5B EBITDA at 9x), asymmetry at $164 is ~1.1x. **PJM "adds" therefore route to CEG and NRG** (the genuine 2x asymmetry names), not VST. Any VST add is gated on completing the bear re-underwrite.
2. **LRCX PT = $450** (per [[LRCX]] note + [[Verification Worklist]], 6/22). The [[Position Sizing Framework]] still shows $380 — fix it there.
3. **ASML's Q2 is a PT-reset event.** PT $1,730 is stale (below the $1,927 price at a 52-week high), so the branches drive an explicit PT decision rather than "no action."
...
|---|---|---|
| EUV ≥60 + guide raised | Supercycle confirmed; PT stale to the upside | **Restate PT to ≥$1,930** (at/above price); reassess Hold toward at-market; positive read-through confirms LRCX/AMAT/TOELY |
| In-line | Valuation already full | **Reset PT to ~$1,900–1,950** band; stay Hold |
| EUV cut or pushed | Supercycle wobble | Hold; PT down; check the shared **>15% WFE sell trigger** (LRCX/AMAT/TOELY) — one print isn't 15%, two peers confirming is; prep [[LRCX]] questions |
...
| Outcome | Read | Ratified action |
|---|---|---|
| Backlog ≥105GW + spread intact | On track for 110GW; each GW >110 ≈ +$15/share | **Raise PT** (overrides the harvest rule); stay full weight |
| Backlog ~flat (~100GW) **and** stock ≥$1,140 | Order pace stalling + within 5% of PT | **Harvest to Tier 3** per downsize rule; hard Q3 checkpoint (two flat quarters = re-underwrite) |
| Backlog declines / cancellations / deposit forfeitures in 10-Q | **Primary sell trigger fired** | Re-underwrite immediately; default-exit if a 2nd trigger (capex cuts) is also live |
| Equipment EBITDA margin <11% | Old-to-new price conversion failing (bear trigger #3) | Combine with backlog read before acting |
...
| Outcome | Read | Ratified action |
|---|---|---|
| Revenue ≥$6.8B + Q1 FY2027 guide ≥$7.0B | Supercycle accelerating; HBM4/NAND pulling forward | Consider PT raise above $450; log same day |
| Revenue $6.2–$6.8B + Q1 guide ≥$6.5B | In-line; thesis intact | Confirm PT $450; **fix the $380 in [[Position Sizing Framework]]** |
| Revenue <$6.2B **or** Q1 guide flat/down | Demand softening | No add; two consecutive misses = re-underwrite |
| China share <20% + no offset commentary | Export-control risk activating (invalidation trigger #3) | Re-underwrite immediately per bear case |
| ≥2 of 3 memory customers (Samsung/SK Hynix/Micron) signal >20% YoY capex cut | Shared WFE downgrade trigger | Downgrade per bear case; negative read-through AMAT/TOELY/ASML |
...
| Clearing price | Read | Ratified action |
|---|---|---|
| **> $325/MW-day** | Structural tightening confirmed | **Add to CEG and NRG** (real 2x asymmetry + variant perception → upsize one tier); revisit CEG/NRG PTs upward; **VST add only if its bear re-underwrite is complete and survives**; log same day |
| **$270 – $325** | In-line; thesis intact | No action; confirm PTs unchanged |
| **< $270** (below prior) | Reserve-margin thesis weakening | Re-underwrite PJM leg of VST/CEG within 1 week; if PT support drops >15%, downgrade per invalidation discipline |
| Auction delayed / FERC CIFP rule change | Regulatory risk activated | Check **CEG's #1 invalidation condition** (FERC ruling against co-location) before any action |
...
| Outcome | Read | Ratified action |
|---|---|---|
| Cogentrix closes on terms **+ PJM cleared >$325** | Best risk/reward in book — *if* bear re-underwrite holds | Consider Tier-1 weight (only after bear restated and asymmetry re-scored) |
| Cogentrix closes, PJM in-line | De-risked | Hold at Tier 2; recheck Q3 |
| Cogentrix delayed (not repriced) | Timeline risk only | Hold; recheck Q3 |
| Cogentrix repriced or fails | **Sell trigger fired** | Re-underwrite immediately |
...
After each event: 1) record outcome + action in [[Decisions Log]] same week; 2) draft the next quarter's playbook entries at earnings-prep time (automated Monday scan flags reporters 14 days out). The **catalyst-execution** scheduled task (weekday 7am) reads this file, matches each resolved event to its branch, and drafts the action + Decisions Log entry to `08_Monitors/`.
